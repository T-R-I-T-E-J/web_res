...
COMMENT ON COLUMN public.news_articles.featured_image_url IS 'DEPRECATED: Use image_urls instead. Kept for backward compatibility.';
COMMENT ON COLUMN public.news_articles.image_urls IS 'Array of image URLs for the news article. First image is considered the featured image.';
-- Step 4: Create index for better query performance
CREATE INDEX IF NOT EXISTS idx_news_articles_image_urls ON public.news_articles USING GIN (image_urls);
